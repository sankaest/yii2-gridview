...
class ActionColumn extends YiiActionColumn
{
    ...
    public function init()
    {
        ...
        if (!isset($this->header)) {
            $this->header = Yii::t('vkgrid', 'Actions');
        }
        $this->parseFormat();
    ...
    protected function setDefaultButton($name, $title, $icon)
    {
        ...
        $this->buttons[$name] = function ($url) use ($name, $title, $icon) {
            ...
            if ($name === 'delete') {
                $item = isset($this->grid->itemLabelSingle) ? $this->grid->itemLabelSingle : Yii::t('vkgrid', 'item');
                $options['data-method'] = 'post';
                $options['data-confirm'] = Yii::t('vkgrid', 'Are you sure to delete this {item}?', ['item' => $item]);
            }
            $options = array_replace_recursive($options, $this->buttonOptions, $this->$opts);
            $label = $this->renderLabel($options, $title, ['class' =>  $icon, 'aria-hidden' => 'true']);
//            $label = $this->renderLabel($options, $title, ['class' => $this->grid->getDefaultIconPrefix() . $icon, 'aria-hidden' => 'true']);
    ...
    {
        $this->setDefaultButton('view', Yii::t('vkgrid', 'View'), 'eye');
        $this->setDefaultButton('update', Yii::t('vkgrid', 'Update'), 'pencil-alt');
        $this->setDefaultButton('delete', Yii::t('vkgrid', 'Delete'), 'trash-alt');
    }
    ...
    protected function renderDataCellContent($model, $key, $index)
    {
        ...
        if ($this->_isDropdown && !empty($trimmed)) {
            $label = ArrayHelper::remove($options, 'label', Yii::t('vkgrid', 'Actions'));
            $options = array_replace_recursive($options, ['type' => 'button', 'data-toggle' => 'dropdown']);
            Html::addCssClass($options, 'dropdown-toggle');
            $button = Html::button($label, $options);
            Html::addCssClass($this->dropdownMenu, 'dropdown-menu');
            $dropdown = $button . PHP_EOL . Html::tag('div', $content, $this->dropdownMenu);
            Html::addCssClass($this->dropdownOptions, 'btn-group');
