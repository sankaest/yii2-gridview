...
class GridView extends YiiGridView implements BootstrapInterface
{
     ...
     */
//    public $krajeeDialogSettings = []; // peaks saama kasutada native BS
...
HTML;
     ...
     * ```php
     * [
     *      ['from'=>GridView::ICON_ACTIVE, 'to'=>Yii::t('vkgrid', 'Active')],
     *      ['from'=>GridView::ICON_INACTIVE, 'to'=>Yii::t('vkgrid', 'Inactive')]
     * ]
     * ```
     */
    ...
    /**
     ******************************** RADIO ************************************
     * @var boolean whether init radio selection option by API
     */
    public $radioSelectionExist = false;
    /**
     * @var string class of row selected by radio input.
     */
    public $radioSelectedRowClass;
    /**
     * @var string name of radio input
     */
    public $radioSelectionName;
    /**
     * @var string name of radio input
     */
    public $radioSelectionClearClass;

    /**
     ******************************** CHECKBOX *********************************
     * @var boolean whether init radio selection option by API
     */
    public $checkSelectionExist = false;
    /**
     * @var string class of row selected by radio input.
     */
    public $checkSelectedRowClass;
    /**
     * @var string name of radio input
     */
    public $checkSelectionName;
    /**
     * @var string name of radio input
     */
    public $checkSelectionClearClass;
    ...
    public function init()
    {
        ...
        if (!isset($this->itemLabelSingle)) {
            $this->itemLabelSingle = Yii::t('vkgrid', 'item');
        }
        if (!isset($this->itemLabelPlural)) {
            $this->itemLabelPlural = Yii::t('vkgrid', 'items');
        }
        if (!isset($this->itemLabelFew)) {
            $this->itemLabelFew = Yii::t('vkgrid', 'items-few');
        }
        if (!isset($this->itemLabelMany)) {
            $this->itemLabelMany = Yii::t('vkgrid', 'items-many');
        }
        if (!isset($this->itemLabelAccusative)) {
            $this->itemLabelAccusative = Yii::t('vkgrid', 'items-acc');
        }

        Html::addCssClass($this->options, 'kv-grid-bs4');

        $this->options['data-widget'] = 'gridview';
        Html::addCssClass($this->options, 'kv-grid-bs4');
    ...
    public function renderPageSummary()
    {
        ...
        if (!isset($this->pageSummaryRowOptions['class'])) {
            $this->pageSummaryRowOptions['class'] = 'table-warning kv-page-summary';
        }
    ...
    public function renderSummary()
    {
        ...
        if ($pagination !== false) {
            ...
            if (($summaryContent = $this->summary) === null) {
                return Html::tag($tag, Yii::t('vkgrid',
            ...
            if (($summaryContent = $this->summary) === null) {
                return Html::tag($tag,
                    Yii::t('vkgrid', 'Total <b>{count, number}</b> {count, plural, one{{item}} other{{items}}}.',
                        $configSummary + $configItems
                    ), $summaryOptions);
            }
        }
    ...
    protected function initExport()
    {
        ...
        $this->exportConversions = array_replace_recursive(
            [
                ['from' => self::ICON_ACTIVE, 'to' => Yii::t('vkgrid', 'Active')],
                ['from' => self::ICON_INACTIVE, 'to' => Yii::t('vkgrid', 'Inactive')],
            ],
        ...
        $this->export = array_replace_recursive(
            [
                ...
                'messages' => [
                    ...
                    'confirmDownload' => Yii::t('vkgrid', 'Ok to proceed?'),
                    'downloadProgress' => Yii::t('vkgrid', 'Generating the export file. Please wait...'),
                    'downloadComplete' => Yii::t(
                        'vkgrid',
                        'Request submitted! You may safely close this dialog after saving your downloaded file.'
                    ),
                ],
                'options' => ['class' => 'btn ' . $this->_defaultBtnCss, 'title' => Yii::t('vkgrid', 'Export')],
        ...
        if (!isset($this->export['header'])) {
            $this->export['header'] = '<li role="presentation" class="dropdown-header">' .
                Yii::t('vkgrid', 'Export Page Data') . '</li>';
        }
        if (!isset($this->export['headerAll'])) {
            $this->export['headerAll'] = '<li role="presentation" class="dropdown-header">' .
                Yii::t('vkgrid', 'Export All Data') . '</li>';
        }
        $title = empty($this->caption) ? Yii::t('vkgrid', 'Grid Export') : $this->caption;
        $pdfHeader = [
            'L' => [
                'content' => Yii::t('vkgrid', 'Yii2 Grid Export (PDF)'),
                'font-size' => 8,
                'color' => '#333333',
            ],
            'C' => [
            ...
            'R' => [
                'content' => Yii::t('vkgrid', 'Generated') . ': ' . date('D, d-M-Y'),
                'font-size' => 8,
                'color' => '#333333',
            ],
        ];
        $pdfFooter = [
            'L' => [
                'content' => Yii::t('vkgrid', '© Krajee Yii2 Extensions'),
                'font-size' => 8,
                'font-style' => 'B',
                'color' => '#999999',
            ],
        ...
        $defaultExportConfig = [
            self::HTML => [
                ...
                'showFooter' => true,
                'showCaption' => true,
                'filename' => Yii::t('vkgrid', 'grid-export'),
                'alertMsg' => Yii::t('vkgrid', 'The HTML export file will be generated for download.'),
                'options' => ['title' => Yii::t('vkgrid', 'Hyper Text Markup Language')],
                'mime' => 'text/plain',
                'cssStyles' => $cssStyles,
                'config' => [
                    'cssFile' => [
            ...
            self::CSV => [
                ...
                'showCaption' => true,
                'filename' => Yii::t('vkgrid', 'grid-export'),
                'alertMsg' => Yii::t('vkgrid', 'The CSV export file will be generated for download.'),
                'options' => ['title' => Yii::t('vkgrid', 'Comma Separated Values')],
            ...
            self::TEXT => [
                ...
                'showCaption' => true,
                'filename' => Yii::t('vkgrid', 'grid-export'),
                'alertMsg' => Yii::t('vkgrid', 'The TEXT export file will be generated for download.'),
                'options' => ['title' => Yii::t('vkgrid', 'Tab Delimited Text')],
                'mime' => 'text/plain',
                'config' => [
                    'colDelimiter' => "\t",
                    'rowDelimiter' => "\r\n",
            ...
            self::EXCEL => [
                ...
                'showCaption' => true,
                'filename' => Yii::t('vkgrid', 'grid-export'),
                'alertMsg' => Yii::t('vkgrid', 'The EXCEL export file will be generated for download.'),
                'options' => ['title' => Yii::t('vkgrid', 'Microsoft Excel 95+')],
                'mime' => 'application/vnd.ms-excel',
                'cssStyles' => $cssStyles,
                'config' => [
                    'worksheet' => Yii::t('vkgrid', 'ExportWorksheet'),
                    'cssFile' => '',
                ],
            ],
            self::PDF => [
                'label' => Yii::t('vkgrid', 'PDF'),
                ...
                'showCaption' => true,
                'filename' => Yii::t('vkgrid', 'grid-export'),
                'alertMsg' => Yii::t('vkgrid', 'The PDF export file will be generated for download.'),
                'options' => ['title' => Yii::t('vkgrid', 'Portable Document Format')],
                ...
                'config' => [
                    ...
                    'options' => [
                        'title' => $title,
                        'subject' => Yii::t('vkgrid', 'PDF export generated by kartik-v/yii2-grid extension'),
                        'keywords' => Yii::t('vkgrid', 'krajee, grid, export, yii2-grid, pdf'),
                    ],
                    'contentBefore' => '',
                    'contentAfter' => '',
                ],
            ...
            self::JSON => [
                ...
                'showCaption' => true,
                'filename' => Yii::t('vkgrid', 'grid-export'),
                'alertMsg' => Yii::t('vkgrid', 'The JSON export file will be generated for download.'),
                'options' => ['title' => Yii::t('vkgrid', 'JavaScript Object Notation')],
    ...
    protected function initToggleData()
    {
        ...
        $defaultOptions = [
            ...
            'all' => [
                'icon' => 'fas fa-expand',
                'label' => Yii::t('vkgrid', 'All'),
                'class' => $defBtnCss,
                'title' => Yii::t('vkgrid', 'Show all data'),
            ],
            'page' => [
                'icon' => 'fas fa-compress',
                'label' => Yii::t('vkgrid', 'Page'),
                'class' => $defBtnCss,
                'title' => Yii::t('vkgrid', 'Show first page data'),
            ],
        ];
    ...
    protected function initBootstrapStyle()
    {
        ...
        if ($this->condensed) {
            Html::addCssClass($this->tableOptions, self::BS_TABLE_CONDENSED);
        }
        if ($this->floatHeader) {
    ...
    protected function initLayout()
    {
        ...
        // init Radio Selection JS API
        if($this->radioSelectionExist) {
            $this->options['data-grid-radio-class'] = $this->radioSelectedRowClass;
            $this->options['data-grid-radio-name'] = $this->radioSelectionName;
            $this->options['data-grid-radio-clear'] = $this->radioSelectionClearClass;
        }

        // init Checked Selection JS API
        if($this->checkSelectionExist) {
            $this->options['data-grid-check-class'] = $this->checkSelectedRowClass;
        }
    ...
    protected function initPanel()
    {
        ...
        if (isset($this->panelPrefix)) {
            ...
            Html::addCssClass($options, self::BS_PANEL);
            Html::addCssClass($options, "border-{$type}");
        }
        static::initCss($summaryOptions, self::BS_PULL_RIGHT);
        $titleTag = ArrayHelper::remove($titleOptions, 'tag', 'h5');
        static::initCss($titleOptions, 'm-0');
        if ($heading !== false) {
            $color = $type === 'default' ? ' bg-light' : " text-white bg-{$type}";
            static::initCss($headingOptions, self::BS_PANEL_HEADING . $color);
            $panelHeading = Html::tag('div', $this->panelHeadingTemplate, $headingOptions);
        }
        if ($footer !== false) {
            static::initCss($footerOptions, self::BS_PANEL_FOOTER);
    ...
    protected function renderToolbarContainer()
    {
         ...
         */
        if (
            !strpos($this->toolbarContainerOptions['class'], self::BS_PULL_RIGHT)
            && !strpos($this->toolbarContainerOptions['class'], self::BS_PULL_LEFT)
        ) {
            Html::addCssClass($this->toolbarContainerOptions, self::BS_PULL_RIGHT);
        }
